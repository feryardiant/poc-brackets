...
export class Match {
   ...
   * @returns {import('./parties').Party} Party obj based on match
   */
  toParty(index, matches) {
    ...
    const party = new Party(name, id, this.next.round, {
      id: this.id,
      round: this.round,
      size: this.size,
      span: this.next.span,
    ...
    party.side = this.next.side
    party.order = index + 1

    return party.normalize(matches.length + this.round, this.next.side)
  }
}
