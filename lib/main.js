...
function determineNextRoundFromPrevMatches(matches, match) {
  ...
  for (const party of match.parties) {
    ...
    if (prevAfterMatch.next.gap > 0 && prevAfterMatch.next.side === 'red') {
    ...
    if (prevAfterMatch.round === party.match.round && !hasSingularParty) {
      match.gap++
    }
  }
}
...
function determineNextRoundForCurrentMatch(round, singular, matches, index, total) {
  ...
  if (prevOne.next.round === next.round && prevOne.next.side === next.side) {
    next.side = {
      blue: 'red',
      red: 'blue',
    }[next.side]
  ...
  // Ensure last match on the first round should next be red side
  if (round === 1) {
    ...
    if (index > 0 && prevTwo?.singular === true) {
      next.side = 'red'
      prevTwo.next.gap++
      next.round++
      next.gap++
...
function createMatches(parties, roundId, fnId = num => num) {
  ...
  if ([7, 9, 11, 13].includes(sidedMatches.length)) {
    half++
  }
  ...
  return splits.reduce((matches, split) => {
    split.forEach((side, index, split) => {
      ...
      if (prevMatch?.gap > 0) {
        ...
        if (
          prevMatch.parties.some(p => p.match.singular)
          && (match.next.side === 'red' && match.parties.every(p => p.match.singular))
        ) {
          // prevMatch.next.gap++
          // match.next.round++
          // match.next.gap++
