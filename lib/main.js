import { generateParties, rearrangePartiesBySide } from './parties.js'
 ...
 * @returns {import('./types').MatchToPartyFn} Match to participant callback
 */
function matchWinnerAsNextParty(matches) {
  return (match, m) => {
    ...
    const party = {
      ...
      order: ((m + 1) / (matches.length + match.round)).toPrecision(3),
      match: {
        id: match.id,
        round: match.round,
        gap: match.next.gap,
...
function createMatches(parties, round, fnId = num => num) {
  ...
  return splits.reduce((matches, split) => {
    split.forEach((side, index, split) => {
      ...
      // increase the gap of current match based on either parties
      ...
      // previous match has greater gap and `next.side` target was `red`
      if (prevMatch?.gap > 0 && match.next.side === 'red') {
        match.next.round++
      }
