...
function determineNextRoundForCurrentMatch(round, singular, matches, index, total) {
  ...
  const twoPrevMatchesFaceOnNextRound = prevTwo?.next.side === 'blue'
    && (prevOne.next.side !== prevTwo.next.side && prevOne.next.round === prevTwo.next.round)
  ...
  if (round === 1) {
    if (next.side === 'red' && next.round === round && twoPrevMatchesFaceOnNextRound) {
      prevTwo.next.gap++
      next.round++
      next.gap++
    }
