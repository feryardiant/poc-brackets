 ...
 */
function createMatches(parties, round, fnId = num => num, matches = [], byes = []) {
...
export function render($chart, parties) {
  ...
  $chart.addEventListener('winner-selected', e => winnerSelected(e.detail, rounds))
      ...
      for (const party of match.parties) {
        ...
        if (!match.singular) {
          ...
          $check.addEventListener('change', (e) => {
            ...
            const $el = e.target
            const winner = match.parties.find(party => party.id === Number($el.value))

            $chart.dispatchEvent(new CustomEvent('winner-selected', {
              detail: { $el, winner, match, round },
            }))
          })
        }
...
/**
 * @typedef {object} WinningParam
 * @property {HTMLInputElement} $el
 * @property {import('./types').Round} round
 * @property {import('./types').Match} match
 * @property {import('./types').Party} winner
 *
 * @param {WinningParam} param0
 * @param {import('./types').Round[]} rounds
 */
function winnerSelected({ $el, match, winner }, rounds) {
  const flip = { red: 'blue', blue: 'red' }
  ...
  if (Array.from($parties).every($radio => $radio.disabled)) {
    $matches.classList.add('completed')
    enableParties($matches.parentElement.nextElementSibling)
  }

  // Mark the opponent as loser
  $matches.querySelector(
    `div[data-match="${match.id}"] [data-side=${flip[winner.side]}]`,
  ).classList.add('loser')
  ...
  const $party = document.getElementById(
    `round-${match.next.round}-match-${nextMatch.id}`,
